//! Pure-Rust implementation of the `SB04MD` Sylvester solver subset.
...
/// Errors returned by the pure-Rust `SB04MD` implementation.
#[derive(Debug, Error)]
pub enum Sb04MdError {
...
/// Output bundle for the pure-Rust `SB04MD` implementation.
#[derive(Clone, Debug, PartialEq)]
pub struct Sb04MdResult {
    pub x: Vec<Vec<f64>>,
    pub z: Vec<Vec<f64>>,
}
...
/// # Errors
///
/// Returns [`Sb04MdError`] if the input dimensions are incompatible or the
...
pub fn sb04md_solve(
    ...
    if rhs_matrix.len() != left_order || rhs_matrix.iter().any(|row| row.len() != right_order) {
        return Err(Sb04MdError::IncompatibleRightHandSide {
    ...
    let kronecker_system = build_sylvester_system(left_matrix, right_matrix);
...
fn build_sylvester_system(a: &[Vec<f64>], b: &[Vec<f64>]) -> Vec<Vec<Complex64>> {
    ...
    let identity_left = identity(left_order);
    let identity_right = identity(right_order);
    let left = kronecker(&identity_right, a);
    let right = kronecker(&transpose(b), &identity_left);
    add_real_matrices(&left, &right)
}

fn schur_vectors_of_transpose(b: &[Vec<f64>]) -> Vec<Vec<f64>> {
    let order = b.len();
    let transpose_b = transpose(b);
...
#[cfg(test)]
mod tests {
    use super::sb04md_solve;

    #[test]
    fn solves_scalar_sylvester_case() {
        let a = vec![vec![2.0]];
        let b = vec![vec![3.0]];
        let c = vec![vec![10.0]];
        let result = sb04md_solve(&a, &b, &c).expect("solve should succeed");

        assert!((result.x[0][0] - 2.0).abs() < 1.0e-12);
    }
}
