//! Golden test: SB03TD (continuous Lyapunov) via SB03MD.

use std::path::{Path, PathBuf};

use slicot_routines::sb03md_solve;
use slicot_test_harness::load_sb03td_case;
...
#[test]
fn pure_rust_sb03td_via_sb03md_matches_upstream_fixture() {
    let case = load_sb03td_case(examples_root()).expect("SB03TD fixture should parse");
    ...
    assert!(
        (result.scale - case.output.scale).abs() < 1.0e-6,
        "scale rust={} ref={}",
        result.scale,
        case.output.scale
    );
